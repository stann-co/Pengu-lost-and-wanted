...
if(controlled){
	...
	if((!airborne && ground_spd < 0) || airborne) { //left
		...
		//if(push_sensor == noone) {
		//	push_sensor = sensor(vec_lt,-90,sensor_length_base,abs(x_speed));	
		//}
		
		if(push_sensor != noone && push_sensor.distance < 1 && !sensor_is_oneway(push_sensor)){		
			if(sensor_is_destructible(push_sensor)){
	...
	if((!airborne && ground_spd > 0) || airborne) { //right
		...
		//if(push_sensor == noone) {
		//	push_sensor = sensor(vec_rt,90,sensor_length_base,abs(x_speed));	
		//}
	...
	if(!airborne || (airborne && y_speed > 0)){
		...
		var sensor_length = sensor_length_base//? 2 : sensor_length_base;
