...
export type PromiseUnwrapped<T> = T extends PromiseLike<infer U> ? PromiseUnwrapped<U> : T;
...
export type ExtractKeysByValue<Container, ValueTypeFilter> = {
    [Key in keyof Container]-?: Container[Key] extends AnyFunction ? ValueTypeFilter extends Container[Key] ? Key : never : Container[Key] extends ValueTypeFilter ? Key : never;
}[keyof Container];
export type ExcludeKeysByValue<Container, ValueTypeFilter> = Exclude<keyof Container, ExtractKeysByValue<Container, ValueTypeFilter>>;
export type PickByValue<Container, ValueTypeFilter> = Pick<Container, ExtractKeysByValue<Container, ValueTypeFilter>>;
export type OmitByValue<Container, ValueTypeFilter> = Omit<Container, ExtractKeysByValue<Container, ValueTypeFilter>>;
 ...
 * ```
 */
export type CompoundLookup<T extends Record<string, any>, K extends (keyof T)[]> = K extends [infer First, ...infer Rest] ? First extends string ? T[First] extends string ? {
    [Value in T[First]]: Rest extends (keyof T)[] ? CompoundLookup<T, Rest> : never;
} : never : never : T;
interface RegExpExecArray {
    indices?: RegExpIndicesArray;
}
interface RegExpIndicesArray<Groups extends string = string> extends Array<[number, number]> {
    groups?: {
        [Name in Groups]: [number, number];
    };
}
export interface RegExpExecArrayWithIndices<Groups extends string = string> extends RegExpExecArray {
