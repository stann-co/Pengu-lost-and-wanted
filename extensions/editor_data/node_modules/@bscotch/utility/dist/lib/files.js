 ...
 */
export function removeEmptyDirsSync(startDir, options) {
 ...
 * (e.g. imports)
 */
export function asModuleString(data, type, preamble) {
    return `${preamble ? `${preamble}\n\n` : ''}export default ${JSON.stringify(data)}${type ? ` as ${type}` : ''};\n`;
}
/**
 ...
 * (e.g. imports)
 */
export function writeAsModule(data, filePath, type, preamble) {
    const ext = path.extname(filePath);
    if (!['.js', '.ts'].includes(ext)) {
        throw new Error(`${filePath} must have a ts or js extension`);
    }
    const asString = asModuleString(data, ext == '.ts' ? type : undefined, preamble);
...
export function writeJsonFileSync(filePath, data, spaces = 2, replacer) {
 ...
 */
export function getModuleFilepath(meta) {
    if (!meta?.url) {
 ...
 */
export function getModuleDir(meta) {
 ...
 * ```
 */
export function findParentByFileSync(findFile, startDir = process.cwd()) {
