 ...
 */
export function markdownTableToObjects(table, options) {
    ...
    const headers = rows
        .shift()
        ?.map((header, headerIndex) => options?.transformColumnName
        ? options.transformColumnName(header, headerIndex)
        : header);
    assert(headers?.length, 'No headers found');
    const headerBreakRow = rows.shift();
    assert(headerBreakRow.every((col) => col.match(/^-+$/)), 'Header break row is not all dashes');
    const asObjects = rows.map((row, rowNumber) => {
        return headers.reduce((obj, header, columnNumber) => {
            ...
            const value = options?.transformCellContent
                ? options.transformCellContent(row[columnNumber], rowNumber, columnNumber, header)
                : row[columnNumber];
            if (value === undefined) {
                return obj;
            }
