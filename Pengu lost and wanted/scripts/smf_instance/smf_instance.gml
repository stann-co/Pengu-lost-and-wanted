...
function smf_instance(_model) constructor 
{
    ...
    ///Added by STANN.co
    /// @func get_bone_names() 
    static get_bone_names = function()
    {
        //returns array of all bone names
        return ds_map_keys_to_array(model.rig.boneNames)
    }
    
    ///Added by STANN.co
    static get_bones = function()
    {
        //returns array of all bones
        return ds_map_values_to_array(model.rig.boneNames)
    }
    
    ///Added by STANN.co
    /// @func node_get_id(bone_name) 
    static node_get_id = function(bone_name)
    {
        //returns node id from bone_name
        return model.rig.boneNames[?bone_name]
    }
    
    ///Added by STANN.co
    /// @func node_translate (node,xx,yy,zz,transformChildren) 
    static node_translate = function(node,xx,yy,zz,transformChildren)
    {
        sample_node_translate(model.rig, node, sample, xx,yy,zz,transformChildren)
    }
    ...
    /// @func node_get_room_pos(node) 
	static node_get_room_pos = function(node,world_mat) 
	{	/*Returns the world-space position of the node as an array of the following format:
			[x, y, z];
            Use the same matrix used to move the instance in the world
        */
        var mat = matrix_multiply(node_get_matrix(node),world_mat)
