...
function CatspeakGMLCompiler(ir, interface=undefined) constructor {
    ...
    /// @ignore
    self.allowedObjects = __catspeak_is_nullish(interface) ? { } : interface.getAllowedObjects();
...
function __catspeak_expr_loop_with__() {
    ...
    if (!__catspeak_is_withable(scope_, allowedObjects)) {
...
function __catspeak_expr_call_method__() {
    ...
    } else if (__catspeak_is_withable(collection_, allowedObjects)) {
        callee_ = collection_[$ key_];
    } else {
