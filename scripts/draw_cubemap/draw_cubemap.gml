...
function draw_tilemap_cubemap(_tilemap,_texture,_n_strength = 0.5,_r_strength = 0.6 ,_scale = 1 ,_offset_scale = -0.25, _opacity = 1.0){
	if(global.draw_shine){
		...
		//var cw = global.camera.width;
		//var ch = global.camera.height;
		//for( var th=cell_x, xx = x_; xx < ww; ++th, xx+=tile_size) {
		//   for( var tv=cell_y, yy = y_; yy < ch; ++tv, yy+=tile_size) {
		//       var data = tilemap_get( _tilemap, th, tv );
		//       draw_tile( tileset, data, 0, xx, yy );
		//   }
		//}
